...
이 문서는 `MVC`/`Json`/`Monitor`/`Dummy` 4개 PoC에서 검증된 설계를 바탕으로,
`Main`의 반도체 시료 생산주문관리 콘솔 시스템을 phase별로 통합 개발하기 위한
전체 계획이다. 상위 도메인 모델은 `../../CLAUDE.md`, 이 디렉토리의 목표/메뉴
명세는 `../CLAUDE.md`를 따른다.
...
- `../CLAUDE.md` — Main의 목표/메뉴 명세/평가 기준
- `../../CLAUDE.md` — 전체 시스템 공통 가이드/도메인 모델
- `../../MVC/README.md`, `../../MVC/docs/Plan.md` — MVC 계층 분리 설계
- `../../Json/README.md`, `../../Json/docs/Plan.md` — JSON 스키마/Repository 정책
- `../../Monitor/README.md`, `../../Monitor/docs/Plan.md` — 집계/판정 로직
- `../../Dummy/README.md`, `../../Dummy/docs/Plan.md` — 더미 생성/ID 채번 로직
