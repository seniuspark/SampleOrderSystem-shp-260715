...
7. 초기 데모 데이터로 앱을 실행해, 다음 시나리오가 문서에 기록된 절차대로
   기대한 결과를 낸다: 시료 등록 확인 → 신규 주문 접수 → 재고 충분 케이스
   승인(즉시 CONFIRMED) → 재고 부족 케이스 승인(PRODUCING + 생산 큐 등록) →
   생산 완료 처리(PRODUCING→CONFIRMED, 재고 반영) → 모니터링에서 상태별 집계/
   재고 판정 확인 → 출고 처리(CONFIRMED→RELEASE) → 거절 케이스(REJECTED가
   모니터링에서 제외됨) 확인.
